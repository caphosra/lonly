...
pub fn unify_exprs<'a, 'b>(
    expr1: &'b Expr<'a>,
    expr2: &'b Expr<'a>,
    constraints: &mut HashMap<VarID, &'b Expr<'a>>,
) -> Result<(), ()> {
    match (expr1, expr2) {
        ...
        (Expr::Var(var1), Expr::Atom(_)) => {
            ...
            if let Some(exp1) = constraints.get(&id) {
                unify_exprs(exp1, expr2, constraints)
            } else {
                if occur_check(expr2, id) {
                    Err(())
                } else {
                    constraints.insert(id, expr2);
                    Ok(())
                }
            }
        }
        ...
        (Expr::Var(var1), Expr::Var(var2)) => {
            ...
            if let Some(exp1) = constraints.get(&id1) {
                ...
                if let Some(exp2) = constraints.get(&id2) {
                    unify_exprs(expr1, exp2, constraints)
                } else {
                    constraints.insert(id1, expr2);
                    Ok(())
                }
            }
        }
...
mod test {
    ...
    fn test_unify1() {
        ...
        assert!(env.assign_new_ids(&mut exprs, &mut assigned).is_ok());
